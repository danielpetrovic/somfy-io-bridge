...
- **`Position`** (default) - the only mode that can send arbitrary percentage targets to the motor. An explicit percentage (confirmed via passive 2W read-back across multiple physical devices/percentages) or a full Open/Close reaching the motor's own mechanical end-stop both land exactly where expected regardless of the travel-time animation's accuracy along the way - see [Travel time](#travel-time) below. A manual **Stop** partway through a move is the one exception: this bridge never gets real position feedback over 1W, so whatever's displayed and persisted afterward is exactly wherever the travel-time estimate happened to be at that instant, not a real reading - see [Travel time](#travel-time) for why that estimate's accuracy still matters here.
- **`Open / My / Close`** - three discrete states (0% closed / 50% "My" / 100% open), no time tracking, no arbitrary percentages - any position request strictly between 0 and 100 collapses to the real My/favorite-position command (see [My and Set My](#my-and-set-my) below). My and Stop are **not** the same command: the dedicated Stop action still sends `Stop` (`main=0xd2`), which only has an effect while the motor is actively moving. Unlike `Position`, this mode's displayed position is driven **entirely** by the travel-time estimate while moving, since Open/Close never carry a percentage at all - see [Travel time](#travel-time).
...
Each cover has **Travel Time Open**/**Travel Time Close** number entities (config entities, seconds) controlling how long the local position estimate takes to run from one extreme to the other. One fixed 25s value couldn't fit every installation (GitHub issue #4 - real stopwatch data showed one axis taking 45s against a 25s model, and another taking 12-15s, on the same board, errors running in opposite directions at once).

This matters differently depending on [mode](#modes) and how a move actually ends. In `Position`, an explicit percentage or a full Open/Close that reaches the real end-stop both land exactly where expected no matter how accurate the animation was along the way, since the motor itself is the source of truth there - purely cosmetic in that case. But a manual **Stop** partway through a move (in either mode) never gets a real position back from the motor at all (no feedback over 1W), so the value shown and persisted afterward is exactly whatever this estimate computed at that instant - a mistuned value means that's a genuinely wrong number, not just a wrong animation. `Open / My / Close` is the same situation for its *entire* range while moving, not just an interrupted Stop, since Open/Close never carry a percentage to begin with - this estimate is the only thing driving what Home Assistant shows until the move either completes (settling correctly at 100%/0%) or gets stopped early.

Starts from the `travel_time_open`/`travel_time_close` YAML substitutions (default 25s each) on first-ever boot; once changed from Home Assistant, the choice persists on the device and the YAML default no longer applies, same mechanism as [Tilt Support](#tilt-support-switch). Both values are always in Home Assistant's own space (what "open" means to HA) regardless of [Direction inversion](#direction-inversion) below - the component handles mapping that onto the motor's real physical direction internally, so a value entered here never needs to be swapped by hand when Invert Direction is toggled.
...
Only the protocol/radio-level pieces were ported from rspaargaren/iohomecontrol specifically (packet framing, CRC/AES handling, position estimation, the 1W command/pairing layer); its MQTT layer, web server, and Arduino `main.cpp` structure were not, and its 2W logic was independently rebuilt against the other two sources instead of being ported from rspaargaren at all - this repository's bridge runs entirely inside ESPHome's own component/lifecycle model.
